...
# --8<-- [start:pl_no_index]
out_pl = (df_pl.select(nrs=pl.col("nrs").sort(),
                       names=pl.col("names").reverse()))
print(out_pl)
# --8<-- [end:pl_no_index]

# --8<-- [start:pd_no_index]
out_pd = (df_pd.assign(nrs=lambda df_: df_.nrs.sort_values().reset_index(drop=True),
...
# --8<-- [start:pd_assign]
out_pd = df_pd.assign(add1=lambda df_: df_.nrs + 1,
